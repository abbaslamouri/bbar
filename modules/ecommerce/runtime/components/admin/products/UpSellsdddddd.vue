...
const localUpSellIds = ref(props.modelValue.upSells.map((p: IProduct) => p._id))
...
const updateModelValueCrossSells = () => {
  const newUpSells = []
  for (const upSellId of localUpSellIds.value) {
    const found = products.value.find((p: IProduct) => p._id == upSellId)
    if (found) newUpSells.push(found)
  }
  emit('update:modelValue', { ...props.modelValue, upSells: newUpSells })
}

const removeUpSells = (categoryId: string) => {
  const index = localUpSellIds.value.findIndex((id: string) => id === categoryId)
  console.log(index)
  if (index !== -1) localUpSellIds.value.splice(index, 1)
  updateModelValueCrossSells()
}

watch(localUpSellIds, () => {
  updateModelValueCrossSells()
})
</script>

<template>
  <div class="section up-sells | flow-l" id="up-sells">
    <h2>Cross Sells</h2>
    <div class="inner">
      <div class="selector">
        <FormsBaseListbox
          v-model="localUpSellIds"
          :selectOptions="
            products.map((c) => {
              return { text: c.name, value: c._id }
            })
          "
          nullOption="Select upsells"
        />
      </div>
      <div>
        <ul role="list" class="">
          <li v-for="product in modelValue.upSells" :key="product._id">
            <div class="image">
              <img :src="`${config.public.s3BaseUrl}/products/${product.gallery[0]}`" :alt="`${product.displayName}`" />
            </div>
            <p class="tooltip">{{ product.name }}</p>
            <button @click.prevent="removeUpSells(product._id)">

...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { insertedId } = await mongoDbClient.db().collection('products').insertOne(body)
    if (!insertedId) throw new AppError(MediaInsertionError)
    await mongoDbClient.db().collection('products').deleteMany()
    return await mongoDbClient.db().collection('products').insertOne(newProduct)
  } catch (err) {
